...
# ========================================================
# Constructor: new
# Initializes the SQLiteOrderRepository with a database handle.
# ========================================================
sub new {
    my ($class, $dbh) = @_;
    my $self = { dbh => $dbh };
...
# ========================================================
# Method: find_all
# Retrieves all orders from the database, including customer 
# and item details. Returns an array reference of order hashrefs.
# ========================================================
sub find_all {
    my ($self) = @_;
...
# ========================================================
# Method: insert
# Inserts a new order into the database and returns the order_id.
# ========================================================
sub insert {
    my ($self, $order) = @_;
    
    my $sth = $self->{dbh}->prepare(
        "INSERT INTO orders (order_number, order_date, customer_id, item_id) VALUES (?, ?, ?, ?)"
    );
...
# Finds an existing order by order_number, customer_id, and item_id.
# If the order doesn't exist, it inserts the order and returns the order_id.
# ========================================================
sub find_or_insert {
    my ($self, $order_number, $order_date, $customer_id, $item_id) = @_;

    my $sth = $self->{dbh}->prepare(
        "SELECT order_id FROM orders WHERE order_number = ? AND customer_id = ? AND item_id = ?"
    );
...
# Deletes orders from the database based on the provided order_ids.
